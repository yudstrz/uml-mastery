...
export interface BuilderData {
    actors: string[];
    usecases: string[];
    mappings: Record<number, number[]>; // actorIndex -> usecaseIndex[]
    relations: Relation[];
}
...
export const visualScenarios: Record<ViewType, Scenario[]> = {
    usecase: [
...
export const builderTemplates: Record<string, Template> = {
    gofood: {
        ...
        mappings: { 0: [0, 1, 2], 1: [3, 4], 2: [2, 3] },
        relations: [{ source: 1, target: 2, type: 'include' }]
    },
    atm: {
        title: 'ATM System',
        actors: ['Nasabah', 'Sistem Bank'],
        usecases: ['Masukkan Kartu', 'Verifikasi PIN', 'Cek Saldo', 'Tarik Tunai', 'Struk Print'],
        mappings: { 0: [0, 1, 2, 3], 1: [1, 2, 3] },
        relations: [{ source: 3, target: 4, type: 'extend' }]
    }
};
